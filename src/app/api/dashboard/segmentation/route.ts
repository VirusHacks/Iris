...
const SEGMENTATION_SERVICE_URL = process.env.FORECAST_SERVICE_URL || "http://localhost:4000";
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Segmentation API] Processing file: ${file.name} (${file.size} bytes)`);
    ...
    console.log(`[Segmentation API] Successfully segmented ${result.length} customers`);

    // Store results in memory cache (simple approach - no database)
    setSegmentationCache(user.user.id, result);

    console.log(`[Segmentation API] Cached ${result.length} customers in memory for user ${user.user.id}`);
