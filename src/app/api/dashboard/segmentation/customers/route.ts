...
export async function GET(request: NextRequest) {
  try {
    ...
    // Add test customer to the beginning of customers array
    customers.unshift(testCustomer);

    customers.forEach((customer: any) => {
      const cluster = customer.cluster_label !== undefined ? customer.cluster_label : -1;
      ...
      const customerRecord = {
        id: customer.customerId || customer.id || `customer-${customersByCluster[cluster].length}`,
        customerId: customer.customerId || customer.id || customer.customer_id || null,
      ...
      clusterStats[cluster].segments[segment] = (clusterStats[cluster].segments[segment] || 0) + 1;
    });
    ...
    Object.keys(clusterStats).forEach((clusterStr) => {
      ...
      // But keep test customer "daksh" at the top of cluster 0
      customersByCluster[cluster].sort((a, b) => {
        // Keep test customer at top of cluster 0
        if (cluster === 0) {
