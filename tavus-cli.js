...
const checkApiKeys = async () => {
  ...
  if (!config.NEXT_PUBLIC_FIRECRAWL_API_KEY) {
    ...
    config.NEXT_PUBLIC_FIRECRAWL_API_KEY = firecrawlKey;
    process.env.NEXT_PUBLIC_FIRECRAWL_API_KEY = firecrawlKey;
  }
...
NEXT_PUBLIC_FIRECRAWL_API_KEY=${config.NEXT_PUBLIC_FIRECRAWL_API_KEY}
...
const crawlWebsite = async (websiteUrl, mode = 'crawl', pages = [], maxPages = 10) => {
  ...
  try {
    const client = new FirecrawlClient({ apiKey: process.env.NEXT_PUBLIC_FIRECRAWL_API_KEY });
    let scrapedPages = [];
    ...
    if (mode === 'crawl') {
      ...
      const crawlResult = await client.crawl(websiteUrl, {
        limit: maxPages,
        scrapeOptions: {
          formats: ['markdown'],
      ...
      if (crawlResult.success && crawlResult.data) {
        ...
        scrapedPages = pages.slice(0, maxPages)
          .map(page => {
            const content = page.markdown || page.content || page.text || '';
            return {
              url: page.metadata?.sourceURL || page.url || websiteUrl,
              content: content
            };
          })
          .filter(page => page.content); // Only include pages with content
      } else if (crawlResult.data) {
        // Handle case where data exists but success might be false
        const pages = Array.isArray(crawlResult.data) ? crawlResult.data : [crawlResult.data];
        scrapedPages = pages.slice(0, maxPages)
          .map(page => {
            const content = page.markdown || page.content || page.text || '';
      ...
      for (const pageUrl of pages) {
        try {
          const scrapeResult = await client.scrape(pageUrl, {
            formats: ['markdown'],
            onlyMainContent: true
          });
          ...
          if (scrapeResult.success) {
            ...
            // Sometimes the data is in scrapeResult.data even if success is false
      ...
      const scrapeResult = await client.scrape(websiteUrl, {
        formats: ['markdown'],
        onlyMainContent: true
      });
      
      // Debug: Check response structure
      console.log(chalk.gray(`\nDebug - Response keys: ${Object.keys(scrapeResult).join(', ')}`));
      console.log(chalk.gray(`Debug - Success: ${scrapeResult.success}`));
      console.log(chalk.gray(`Debug - Has data: ${!!scrapeResult.data}`));
      
      // Handle different response structures
      let data = null;
      if (scrapeResult.data) {
        data = scrapeResult.data;
      } else if (scrapeResult.success && scrapeResult) {
        data = scrapeResult;
      }
      
      if (data) {
        console.log(chalk.gray(`Debug - Data keys: ${Object.keys(data).join(', ')}`));
        const content = data.markdown || data.content || data.text || (typeof data === 'string' ? data : '');
        ...
        if (content && content.trim().length > 0) {
          ...
          console.log(chalk.green(`✓ Successfully scraped content (${content.length} characters)`));
        } else {
          console.log(chalk.yellow(`\n⚠ No content found in response for ${websiteUrl}`));
          console.log(chalk.gray('Full response structure:'));
          console.log(chalk.gray(JSON.stringify(scrapeResult, null, 2).substring(0, 1000)));
        }
      } else {
...
program
  ...
  .action(async () => {
    ...
    const missing = [];
    if (!config.NEXT_PUBLIC_FIRECRAWL_API_KEY) missing.push('NEXT_PUBLIC_FIRECRAWL_API_KEY');
    ...
    if (missing.length === 0) {
      ...
      console.log(chalk.gray('NEXT_PUBLIC_FIRECRAWL_API_KEY=your_firecrawl_key'));
